...
$(document).ready(function (){
   $('#notice').dataTable({
    ...
    /*  $('#notice_filter').prepend('<input type="button" id="save"  value ="저장">'); */
   	...
   	$("#searchinfo").on("click", function () {
    	...
    	document.frm.action = "/lifesaver/searchSaverList";
    	document.frm.submit();
	});
   ...
   $('.appr').on('click', function () {
	   Kakao.init("dbb9810247e1dba19df3907533223753");
		...
		<form name="frm">
			...
			<thead >
				 ...
				 <tr>
					 ...
					 <td>
					 <c:out value="${saverList.saver_grade }"></c:out>
					 <input type="hidden" name="saverGrade" value="<c:out value="${saverList.saver_grade }"></c:out>" />	
					 </td>
					 <td>
					 ...
					 <td>
					 <!-- <form action="/admin_updateAuth" method="post" > -->
					 <c:if test="${saverList.saver_auth == 0 }">
					 	<a href="/lifesaver/admin_updateAuth?saverCode=<c:out value="${saverList.saver_code }"></c:out>"><div class="appr">승인하기</div></a>
					 </c:if>
					 <!-- </form> -->
					 </td>
