...
$(document).ready(function (){
    $('#notice').dataTable({
    	...
    	language: {
            "emptyTable": "데이터가 없어요.",
            "lengthMenu": "페이지당 _MENU_ 개씩 보기"+  '<input type="button" id="admin_saver_grade"  value ="저장">',
            "infoEmpty": "데이터 없음",
    ...
    /*컬럼별 검색  */
     $('#notice_filter').prepend('<select id="select" name="col_select"><option value = saver_code>세이버 코드</option>'
    		 +'<option value = common_code>회원코드</option>'
    		 +'<option value = saver_license>면허번호</option>'
    		 +'<option value = saver_grade>세이버분류</option>'
    		 +'<option value = saver_job>직업</option>'
    		 +'<option value = saver_job_addr>근무지</option>'
    		 +'<option value = saver_rev_try>소생시도 횟수</option>'
    		 +'<option value = saver_rev_succ>소생 성공 횟수</option>'
    		 +'<option value = saver_rev_fail>소생 실패 횟수</option>'
    		 +'<option value = saver_auth>승인상태</option>'
     +'</select>');
    ...
    $('click',function(){
     ...
     /* $('#notice_filter').prepend('<input type="button" id="save"  value ="저장">'); */
   	...
   	$("#searchinfo").on("click", function () {
    	...
    	document.frm.action = "/lifesaver/findSaverList";
    	document.frm.submit();
	});
   	
	$("#admin_saver_grade").on("click", function () {
    	
    	document.frm.method = "post";
    	document.frm.action = "/lifesaver/adminSaveGrade";
    	document.frm.submit();
	});
	...
	<div class="main-container" id="main-container" >
		...
		<div id="sidebar" class="sidebar h-sidebar navbar-collapse collapse" >
			...
			<ul class="nav nav-list"  >
				...
				<li class=" hover">
					<a href="/lifesaver/authorization.page">
				...
				<li class=" hover">
					<a href="/lifesaver/hospital.page">
				...
				<li class=" hover">
					<a href="/lifesaver/building.page">
		...
		<form name="frm">
			<div class="page-content">
				...
				<div class="page-content-area">
					<div class="page-header"  style="float: left; width: 100%; padding:11.5px; " >
						<h1 class="page-title">응급 출동 가드 지표</h1>
					</div>
					<!-- /.page-header -->
					<div class="row">
			...
			<thead >
				 ...
				 <tbody style ="width:100%;  text-align: center; border: 1px solid black">
				 <c:forEach var="saverList" items="${saverList }">
				 <tr>
					 <td>
					 <c:out value="${saverList.saver_code }"></c:out>
					 <input type="hidden" name="saverCode" value="<c:out value="${saverList.saver_code }"></c:out>" />
					 </td>
					 <td>
					 <c:out value="${saverList.common_code }"></c:out>		
					 <input type="hidden" name="commonCode" value="<c:out value="${saverList.common_code }"></c:out>" />			 
					 </td>
					 <td>
					 <c:out value="${saverList.saver_license }"></c:out>
					 <input type="hidden" name="saverLicense" value="<c:out value="${saverList.saver_license }"></c:out>" />					 
					 </td>
					 <td>
					 <c:choose>
					 	<c:when test="${saverList.saver_grade == '1' }"> 
					 		<select name="saverGrade" id="">
					 	...
					 	<c:when test="${saverList.saver_grade == '2' }">
					 		<select name="saverGrade" id="">
					 			<option value="2">2급</option>
					 			<option value="1">1급</option>
					 			<option value="3">3급</option>
					 	...
					 	<c:when test="${saverList.saver_grade == '3' }">
					 		<select name="saverGrade" id="">
					 			<option value="3">3급</option>
					 			<option value="1">1급</option>
					 			<option value="2">2급</option>
					 	...
					 	<c:when test="${saverList.saver_grade == '4' }">
					 		<select name="saverGrade" id="">
					 			<option value="4">4급</option>
					 			<option value="1">1급</option>
					 			<option value="2">2급</option>
					...
					<td>
					 <c:out value="${saverList.saver_job }"></c:out>
					 <input type="hidden" name="saverJob" value="<c:out value="${saverList.saver_job }"></c:out>" />
					 </td>
					 <td>
					 <c:out value="${saverList.saver_job_addr }"></c:out>
					 <input type="hidden" name="saverJobAddr" value="<c:out value="${saverList.saver_job_addr }"></c:out>" />
					 </td>
					 <td>
					 <c:out value="${saverList.saver_rev_try }"></c:out>
					 <input type="hidden" name="saverRevTry" value="<c:out value="${saverList.saver_rev_try }"></c:out>" />
					 </td>
					 <td>
					 <c:out value="${saverList.saver_rev_succ }"></c:out>
					 <input type="hidden" name="saverRevSucc" value="<c:out value="${saverList.saver_rev_succ }"></c:out>" />
					 </td>
					 <td>
					 <c:out value="${saverList.saver_rev_fail }"></c:out>
					 <input type="hidden" name="saverRevFail" value="<c:out value="${saverList.saver_rev_fail }"></c:out>" />
					 </td>
					 <td>
					 <c:out value="${saverList.saver_auth }"></c:out>
					 <input type="hidden" name="saverAuth" value="<c:out value="${saverList.saver_auth }"></c:out>" />
					 </td>
				 </tr>	 
				 </c:forEach>
				 </tbody>
