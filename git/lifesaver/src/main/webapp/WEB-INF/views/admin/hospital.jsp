...
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=119f00e9c183b4e48ccbded04cfb8f47&libraries=services"></script>
   ...
   var hosTable =  $('#notice').dataTable({
    ...
    /*컬럼별 검색  */
     $('#notice_filter').prepend('<select id="select" name="col_select"><option value = saver_code>병원코드</option>'
    		 ...
    		 +'<option value = sit_saver_stat>위도</option>'
    		 +'<option value = sit_saver_stat>경도</option>'
     +'</select>');
    ...
    $('#addRow').on( 'click', function () {
    	window.open("/lifesaver/addrow", "주소 입력", "width=800, height=700, toolbar=no, menubar=no, scrollbars=no, resizable=yes, location=no");
...
<script>
		...
		mapOption = {
		    center: new kakao.maps.LatLng(37.5348569,	126.9508049), // 지도의 중심좌표
		    level: 9 // 지도의 확대 레벨
		};  
